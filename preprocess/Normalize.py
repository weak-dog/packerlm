...
x86_jump_opcodes = ["call", "loop", "loope", "loopz", "loopne", "loopnz", "jmp", "je", "jz", "jne", "jnz", "ja", "jnbe",
                    "jae", "jnb", "jb", "jnae", "jbe", "jna", "jg", "jnle", "jge", "jnl", "jl", "jnge", "jle", "jng",
                    "jc", "jnc", "jo", "jno", "js", "jns", "jp", "jpe", "jnp", "jpo"]
remove_prefix = ["dword", "qword", "oword", "xword", "yword", "zword", "xmmword", "ymmword", "zmmword", "tbyte", "word",
                 "byte", "ptr", "short", "offset"]
...
class Normalizer():
    ...
    def hex_to_dec(self, match, opcode, end_ea):
    ...
    def replace_hex_with_dec(self, assembly_code, opcode, end_ea):
        def replacer(match):
            return self.hex_to_dec(match, opcode, end_ea)

        pattern = r'0x[a-fA-F0-9]+'
        return re.sub(pattern, replacer, assembly_code)

    def normalize_opnd(self, opcode, opnd, end_ea):
        ...
        opnd = opnd.replace("-", " _ ")
        opnd = opnd.replace("*", " * ")
        opnd = self.replace_hex_with_dec(opnd, opcode, end_ea)
        opnd = dec_immediate_pattern.sub("const-normal", opnd)
        opnd = opnd.replace(" ", "")
        if opnd[0] == "_":
            opnd = opnd[1:]
        return opnd.strip()
                ...
                for opnd_str in opnd_strs:
                    ...
                    if ":" in opnd_str:
                        for opnd_split in opnd_str.split(":"):
                            opnd_strs_norm.append(self.normalize_opnd(opcode, opnd_split.strip(), end_ea))
                    else:
                        opnd_strs_norm.append(self.normalize_opnd(opcode, opnd_str.strip(), end_ea))
...
if __name__ == '__main__':
    ...
    if configs["normalize"]:
        normalizer = Normalizer(configs["disasm_dir"], configs["norm_dir"], configs["split_region"],
                                configs["norm_threads"])
