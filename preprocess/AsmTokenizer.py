...
class AsmTokenizer:
    ...
    def encode_insn(self, insn_asm, itype, op_info, eflags):
        ...
        for i in range(len(opnd_strs)):
            opnd_type_ids += [self.opnd_type_vocab.get_id(str(op_info[i][0]))] * len(opnd_strs[i].split()) + [
                self.opnd_type_vocab.pad_id]  # pad for the comma
            reg_id_ids += [self.reg_id_vocab.get_id(str(op_info[i][1]))] * len(opnd_strs[i].split()) + [
                self.reg_id_vocab.pad_id]
            opnd_r_ids += [self.reg_r_vocab.get_id(str(op_info[i][2]))] * len(opnd_strs[i].split()) + [
                self.reg_r_vocab.pad_id]
            opnd_w_ids += [self.reg_w_vocab.get_id(str(op_info[i][3]))] * len(opnd_strs[i].split()) + [
                self.reg_w_vocab.pad_id]
    ...
    def encode_func_iter(self, func_info):
        ...
        masked_func_token_ids = ([self.token_vocab.cls_id] + masked_func_token_ids + [self.token_vocab.sep_id])[
                                :self.max_len]
        masked_func_token_labels = ([self.token_vocab.pad_id] + masked_func_token_labels + [self.token_vocab.pad_id])[
                                   :self.max_len]
        func_insn_type_ids = ([self.itype_vocab.cls_id] + func_insn_type_ids + [self.itype_vocab.sep_id])[:self.max_len]
        func_opnd_type_ids = ([self.opnd_type_vocab.cls_id] + func_opnd_type_ids + [self.opnd_type_vocab.sep_id])[
                             :self.max_len]
        ...
        entity_masked_func_token_ids = ([self.token_vocab.cls_id] + entity_masked_func_token_ids + [
            self.token_vocab.sep_id])[:self.max_len]
        entity_masked_func_token_labels = ([self.token_vocab.pad_id] + entity_masked_func_token_labels + [
            self.token_vocab.pad_id])[:self.max_len]
    ...
    def encode_func_with_insn_boundary(self, func_info):
        ...
        padding_masked_func_token_ids = [self.token_vocab.pad_id for _ in
                                         range(self.max_len - len(masked_func_token_ids))]
        padding_masked_func_token_labels = [self.token_vocab.pad_id for _ in
                                            range(self.max_len - len(masked_func_token_labels))]
        ...
        padding_entity_masked_func_token_ids = [self.token_vocab.pad_id for _ in
                                                range(self.max_len - len(entity_masked_func_token_ids))]
        padding_entity_masked_func_token_labels = [self.token_vocab.pad_id for _ in
                                                   range(self.max_len - len(entity_masked_func_token_labels))]
